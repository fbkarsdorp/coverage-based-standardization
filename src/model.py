...
class Simulator:
    ...
    def sample(self, timesteps=1, return_counts=True):
    ...
    def step(self):
        ...
        self.population = self.rng.choice(
            traits, self.n_agents, p=counts / counts.sum())
        innovators = self.rng.binomial(1, self.mu, self.n_agents).astype(bool)
    ...
    @classmethod
    def load(cls, filepath: str = None, data: dict = None):
...
class DiversityEarlyStopping(EarlyStopping):
    ...
    def diversity(self, model: Simulator):
        ...
        p = x[x > 0] / x.sum()
        return utils.hill_number(x, self.diversity_order, p)
